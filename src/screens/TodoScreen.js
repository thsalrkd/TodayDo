...
export default function TodoScreen(){
  // 데이터 및 기본 UI 상태
  const { todos, tags, saveData, updateData, deleteData, refreshData, } = useData();
  const { user } = useAuth();
  ...
  const todoComplete = async (id) => {
    const targetTodo = todos.find(r => r.id === id);
    if (!targetTodo) return;

    const newCompleted = !targetTodo.completed;

    if (newCompleted && !targetTodo.expGiven) {
       try {
         if (user?.uid) {
            await userService.incrementTodoStats(user.uid);
         }
       } catch (e) {
    ...
    const newExpGiven = (newCompleted && !targetTodo.expGiven) ? true : targetTodo.expGiven;

    // 메인이 완료되면 하위도 모두 완료 처리
    let updatedSubSteps = targetTodo.subs || [];
    if (newCompleted) {
    ...
    try {
      await updateData('todo', id, {
        completed: newCompleted,
        subs: updatedSubSteps
      });
    } catch (e) {
      console.warn('[todoComplete] update failed (ignored)', e);
    }
  };
  ...
  const todoImportant = async (id) => {
    ...
    const newValue = !targetTodo.important;
    ...
    try {
      await updateData('todo', id, { important: newValue });
    } catch (e) {
      console.warn('[todoImportant] update failed (ignored)', e);
    }
  };
  ...
  const todoSelect = (parentId, subId = null) => {
    ...
    if (subId === null) {
      if (newSelected.has(parentId)) {
        ...
        const targetTodo = todos.find(r => r.id === parentId);
        if (targetTodo && targetTodo.subs) {
          targetTodo.subs.forEach(step => {
          ...
          onPress: async () => {
            ...
            try {
              ...
              const subUpdates = Object.keys(subDeletes).map(async (parentId) => {
                ...
                const parent = todos.find(r => r.id === parentId);
                if (parent && parent.subs) {
                  ...
                  if (newSubSteps.length !== parent.subs.length) {
                    return updateData('todo', parentId, { subs:newSubSteps });
                  }
                }
              });
              allPromises.push(...subUpdates);

              const mainDeletes = mainIds.map(id => deleteData('todo', id));
    ...
    if (isMainDetailChecked) {
      Alert.alert("삭제", "이 할 일 전체를 삭제하시겠습니까?", [
        ...
        { text: "삭제", style: "destructive", onPress: async () => {
          await deleteData('todo', detailItem.id);
          setDetailVisible(false);
      ...
      if (selectedDetailSubIds.size > 0) {
        Alert.alert("삭제", "선택한 세부 단계를 삭제하시겠습니까?", [
          ...
          { text: "삭제", style: "destructive", onPress: async () => {
            ...
            await updateData('todo', detailItem.id, { subs:newSubSteps });
  ...
  const handleToggleSubStepList = async (parentId, subStepId) => {
    ...
    await updateData('todo', parentId, { subs:newSubSteps });
  };
  ...
  }, [todos, sortOrder, currentMonth, activeFilter, tempDeletedIds]);
  ...
  const handleSaveTodo = async () => {
    ...
    try {
      const exists = todos.find(r => r.id === editItem.id);
      
      if (exists) {
        await updateData('todo', editItem.id, editItem);
      } else {
        await saveData('todo', editItem);
      }
  ...
  const handleConfirmTag = () => {
    if (isEditDetail && editItem) {
      updateData('todo', editItem.id, { tag: editItem.tag });
